...
from PyQt6.QtWidgets import QApplication, QButtonGroup, QCheckBox, QComboBox, QDoubleSpinBox, QFormLayout, QGroupBox, QHBoxLayout, QInputDialog, QLabel, QMainWindow, QMessageBox, QPushButton, QRadioButton, QScrollArea, QSlider, QSpinBox, QTabWidget, QTextEdit, QVBoxLayout, QWidget
from fishing_worker import BotWorker
from settings import delete_profile, get_language, list_profiles, load_meta, load_profile, load_startup_config, reset_to_factory_default, save_language, save_profile
from translations import DEFAULT_LANGUAGE, SUPPORTED_LANGUAGES, Translator
from region_picker import RegionSelector
from theme import VIEWPORT_BAR_BORDER, VIEWPORT_H, VIEWPORT_W, VIEWPORT_WATER_BORDER, apply_app_theme, dock_window_left
...
class ClickableLabel(QLabel):
    ...
    def update_frame(self, frame):
        ...
        self.setPixmap(QPixmap.fromImage(qt_img).scaled(self.size(), Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
...
class BotGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self._preview_heads = []
        for attr, border, handler in (('viewport_water', VIEWPORT_WATER_BORDER, lambda x, y: self.handle_pipette_click('water', x, y)), ('viewport_bar', VIEWPORT_BAR_BORDER, lambda x, y: self.handle_pipette_click('bar', x, y))):
            col = QVBoxLayout()
            col.setSpacing(4)
            head = QHBoxLayout()
            head.setSpacing(4)
    ...
    def _build_tab_regions(self):
        ...
        self.spin_cast_power.setRange(0.1, 2.0)
    ...
    def _build_hsv_subtab_float(self):
        ...
        for label_key, val, attr in (('h_min', lf[0], 'fh_min'), ('s_min', lf[1], 'fs_min'), ('v_min', lf[2], 'fv_min'), ('h_max', uf[0], 'fh_max'), ('s_max', uf[1], 'fs_max'), ('v_max', uf[2], 'fv_max')):
    ...
    def _build_hsv_subtab_zone(self):
        ...
        for label_key, val, attr in (('h_min', lz[0], 'zh_min'), ('s_min', lz[1], 'zs_min'), ('v_min', lz[2], 'zv_min'), ('h_max', uz[0], 'zh_max'), ('s_max', uz[1], 'zs_max'), ('v_max', uz[2], 'zv_max')):
            mx = 179 if label_key.startswith('h') else 255
    ...
    def on_delete_profile(self):
        ...
        if data == self.PROFILE_FACTORY:
            ...
            return
        reply = QMessageBox.question(self, self.tr('dlg_delete_title'), self.tr('dlg_delete_msg', name=data), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    ...
    def on_reset_factory(self):
        reply = QMessageBox.question(self, self.tr('dlg_reset_title'), self.tr('dlg_reset_msg'), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            self._apply_config_dict(reset_to_factory_default())
    ...
    def update_minigame_config(self):
        self.config['minigame'] = {'target_pct': self.spin_target_pct.value(), 'danger_left_pct': self.spin_danger_left.value(), 'danger_right_pct': self.spin_danger_right.value()}
        self._mark_dirty()

    def update_manual_bar_region(self):
        self.config['bar_region'] = {'left': self.spin_bar_left.value(), 'top': self.spin_bar_top.value(), 'width': self.spin_bar_width.value(), 'height': self.spin_bar_height.value()}
        self._mark_dirty()

    def update_auto_config(self):
        self.config['auto_hsv'] = {'h_tol': self.spin_h_tol.value(), 's_tol': self.spin_s_tol.value(), 'v_tol': self.spin_v_tol.value(), 'adaptive_float': self.chk_adapt_float.isChecked(), 'adaptive_zone': self.chk_adapt_zone.isChecked()}
        self._mark_dirty()
    ...
    def update_ops_config(self):
        self.config['humanize'] = {**self.config.get('humanize', {}), 'enabled': self.chk_humanize.isChecked(), 'cast_delay_min': self.spin_cast_min.value(), 'cast_delay_max': self.spin_cast_max.value()}
        self.config['watchdog'] = {'enabled': self.chk_wd.isChecked(), 'timeout_seconds': self.spin_wd_timeout.value()}
        self.config['auto_bait'] = {**self.config.get('auto_bait', {}), 'enabled': self.chk_bait.isChecked(), 'hotkey': self.combo_bait_key.currentData(), 'every_n_catches': self.spin_bait_every.value()}
        self.config['auto_food'] = {'enabled': self.chk_food.isChecked(), 'hotkey': self.combo_food_key.currentData(), 'interval_minutes': self.spin_food_minutes.value()}
        self._mark_dirty()
    ...
    def update_mask_float(self, mask):
        ...
        self.mask_float_view.setPixmap(QPixmap.fromImage(qt_img).scaled(self.mask_float_view.size(), Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))

    def update_mask_zone(self, mask):
        h, w = mask.shape
        qt_img = QImage(mask.data, w, h, w, QImage.Format.Format_Grayscale8)
        self.mask_zone_view.setPixmap(QPixmap.fromImage(qt_img).scaled(self.mask_zone_view.size(), Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
