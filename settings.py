...
CONFIG_FILE = "config.json"  # legacy; tidak dipakai untuk profil aktif
PROFILES_DIR = "profiles"
META_FILE = os.path.join(PROFILES_DIR, "_meta.json")

# Setelan pabrik (dari konfigurasi yang sudah teruji pengguna)
DEFAULT_CONFIG = {
    "water_region": {"top": 230, "left": 791, "width": 106, "height": 100},
    "bar_region": {"left": 832, "top": 538, "width": 250, "height": 29},
    "cast_power_time": 0.4,
    "auto_cast_power": False,
    "bot_mode": "full_auto",
    "bite_sensitivity": 1,
    "hsv": {
        "lower_float": [19, 24, 36],
        "upper_float": [142, 149, 177],
        "lower_zone": [46, 84, 106],
        "upper_zone": [67, 247, 193],
    },
    "auto_hsv": {
        "h_tol": 31,
        "s_tol": 70,
    ...
    "humanize": {
        ...
        "cast_delay_min": 4.5,
        "cast_delay_max": 6.0,
        "cast_power_variance_pct": 5,
        "bite_delay_max_ms": 450,
        "coordinate_jitter_px": 20,
    },
    "watchdog": {
        "enabled": False,
        "timeout_seconds": 45,
...
_PROFILE_NAME_RE = re.compile(r"^[a-zA-Z0-9_\-\s]{1,48}$")
...
def _strip_removed_keys(cfg: dict) -> dict:
    """Hanya simpan kunci yang masih dipakai aplikasi."""
...
def load_startup_config() -> dict:
    """Konfigurasi saat aplikasi dibuka: profil tersimpan terakhir atau bawaan pabrik."""
    meta = load_meta()
...
# --- Legacy API (GUI lama / skrip) ---
def load_config():
    return load_startup_config()


def save_config(config_data):
    """Tidak menulis otomatis ke disk — gunakan save_profile()."""
    pass
