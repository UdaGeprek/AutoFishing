...
class BotWorker(QThread):
    ...
    def _emit_both_previews(self, water_region, bar_region, lower_float, upper_float, lower_zone, upper_zone, water_label='', bar_label='', water_overlays=None):
        ...
        if water_overlays:
            for fn in water_overlays:
                fn(water_frame)
        if water_label:
            cv2.putText(water_frame, water_label, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        if bar_label:
            cv2.putText(bar_frame, bar_label, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    ...
    def run(self):
        ...
        while self.running:
            ...
            if auto_cast_power:
                ...
                calc_time = 0.2 + dist_px / max_expected_dist * 0.75
            ...
            if self.current_state == self.STATE_PAUSED:
                self._emit_both_previews(water_region, bar_region, lower_float, upper_float, lower_zone, upper_zone, 'PAUSED', 'PAUSED')
                last_ui_update = now
                self.msleep(33)
                continue
                ...
                if self._cast_phase is None:
                    ...
                    self.log_ready.emit(f'[Lempar] Menahan klik {cast_power_time}s ke tengah zona air ({water_center_x}, {water_center_y})...')
                    self._last_activity = time.time()
                ...
                if should_update_ui and self.current_state == self.STATE_CASTING:
                    ...
                    self._emit_both_previews(water_region, bar_region, lower_float, upper_float, lower_zone, upper_zone, phase_text, phase_text)
                    last_ui_update = now
                self.msleep(33)
                ...
                if self._tracked_float_pos is not None:
                    ...
                    spotlight_frame = np.full_like(water_frame, (0, 40, 0))
                ...
                if should_update_ui:
                    ...
                    cv2.putText(water_frame, f'FISHING... ({int(elapsed_time)}s)', (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                    self.water_frame_ready.emit(water_frame)
                    bar_frame = self.vision.capture_screen(bar_region)
                    hsv_bar = cv2.cvtColor(bar_frame, cv2.COLOR_BGR2HSV)
                    mask_zone = cv2.inRange(hsv_bar, lower_zone, upper_zone)
                    self.mask_zone_ready.emit(mask_zone)
                    cv2.putText(bar_frame, 'STANDBY', (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 100, 100), 2)
                    self.bar_frame_ready.emit(bar_frame)
                self.msleep(15)
                ...
                if should_update_ui:
                    ...
                    mask_float = cv2.inRange(hsv_water, lower_float, upper_float)
                    self.mask_float_ready.emit(mask_float)
                    cv2.putText(water_frame, 'MINI-GAME...', (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                    self.water_frame_ready.emit(water_frame)
                self.msleep(15)
                ...
                if should_update_ui:
                    last_ui_update = now
                    self.mask_zone_ready.emit(mask_zone)
                    cv2.putText(bar_frame, 'SEMI-AUTO: WAITING...', (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 255), 2)
                    self.bar_frame_ready.emit(bar_frame)
                    water_frame = self.vision.capture_screen(water_region)
                    hsv_water = cv2.cvtColor(water_frame, cv2.COLOR_BGR2HSV)
                    mask_float = cv2.inRange(hsv_water, lower_float, upper_float)
                    self.mask_float_ready.emit(mask_float)
                    cv2.putText(water_frame, 'SEMI-AUTO: WAITING...', (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 255), 2)
                    self.water_frame_ready.emit(water_frame)
                self.msleep(33)
